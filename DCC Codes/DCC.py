...
def main(args, net=None):
    ...
    optimizer = optim.AdamW([{'params': bias_params, 'lr': 2*args.lr},
                            {'params': nonbias_params},
                            {'params': criterion2.parameters(), 'lr': args.lr},
                            ], lr=args.lr, betas=(0.99, 0.999), amsgrad = True)   
    ...
    new_folder = os.path.join(outputdir, str(args.k))

    # Create the new folder if it doesn't exist
    os.makedirs(new_folder, exist_ok=True)

    # Define the full path to the file
    file_path = os.path.join(new_folder, 'features')

    # Save the file
    sio.savemat(file_path, output)
    return assignment,davis
...
def train(trainloader, net, optimizer, criterion1, criterion2, epoch, use_cuda, _sigma1, _sigma2, _lambda, logger):
    ...
    if logger:
        ...
        print('total_loss: ', losses.avg, 'epoch: ', epoch)
        print('reconstruction_loss: ',losses1.avg, 'epoch: ', epoch)
        print('dcc_loss: ', losses2.avg, 'epoch: ' , epoch)
...
def test(testloader, net, criterion, epoch, use_cuda, _delta, pairs, numeval, flag, logger):
    ...
    if flag or epoch % 10 == 0:
        index, ari, ami, nmi, acc, dbl, n_components, assignment = computeObj(U, pairs, _delta, labels, numeval, original)
        global davis
        davis=dbl
        ...
        if logger:
            ...
            #print('epoch: ', epoch, 'AMI: ', ami)
            #logger.log_value('NMI', nmi, epoch)
            #print('epoch: ', epoch, 'NMI: ', nmi)
            #logger.log_value('ACC', acc, epoch)
            #print('epoch: ', epoch, 'ACC: ', acc)
            logger.log_value('DBL', dbl, epoch)
            print('epoch: ', epoch, 'DBL: ', dbl)
