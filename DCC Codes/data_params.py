...
def extract_sdae_tmd(slope=0.0, dim=10):
    return extractSDAE(dim=[36, 500, 500, 2000, dim], slope=slope)

def extract_sdae_oyster(slope=0.0, dim=2):
    return extractSDAE(dim=[10, 500, 500, 2000, dim], slope=slope)
...
def sdae_tmd(dropout=0.2, slope=0.0, dim=10):
    return SDAE(dim=[36, 500, 500, 2000, dim], dropout=dropout, slope=slope)

def sdae_oyster(dropout=0.2, slope=0.0, dim=2):
    return SDAE(dim=[10, 500, 500, 2000, dim], dropout=dropout, slope=slope)    
...
def load_predefined_net(args, params):
    if args.db == 'mnist':
        ...
        net = sdae_tmd(dropout=params['dropout'], slope=params['reluslope'])
    elif args.db == 'oyster':
        net = sdae_oyster(dropout=params['dropout'], slope=params['reluslope'])    
    else:
...
def load_predefined_extract_net(args):
    ...
    if args.db == 'mnist':
      ...
      net = extract_sdae_tmd(slope=reluslope)       
    elif args.db == 'oyster':
      net = extract_sdae_oyster(slope=reluslope)    
    else:
        raise ValueError("Unexpected database %s" % args.db)
